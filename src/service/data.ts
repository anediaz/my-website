type LanguageType = 'eu'|'fr'|'en'|'es';

interface Language {
  id: LanguageType,
  title: string
}

interface Social {
  id: string;
  url: string;
  className?:string;
  isSvg?:boolean;
}

interface Header {
  name: string;
  mail: string;
  url: Record<LanguageType, string>;
  social:Social[];
}

interface Work {
  url?: string;
  alt: string;
}

interface Talk {
  title: string;
  youtubeId: string;
  date: string;
}

interface More {
  item: string;
  contentItems?: string[];
  items?: string;
  content?:boolean;
}

interface Album {
  name: string;
  id: string;
}

export interface jsonDataProps {
  languages: Language[];
  menuItems: string[];
  skills: string[];
  header: Header;
  works: Record<'developer'|'graphic', Work>;
  talks: Talk[];
  more: More[];
  webSiteInfo: {
    author: string;
    url: string;
  }
}

const LANGUAGES = [
  "eu",
  "fr",
  "en",
  "es",
];



export const jsonData:jsonDataProps = {
  languages: [
    {
      id: "en",
      title: "english"
    },
    {
      id: "fr",
      title: "français"
    },
    {
      id: "es",
      title: "español"
    },
    {
      id: "eu",
      title: "euskera"
    }
  ],
  menuItems: [ "about", "skills", "works", "more"],
  skills: ["developer", "graphic", "speaker"],
  header: {
    name: "Ane DIAZ DE TUESTA",
    mail: "anediaz@gmail.com",
    url: {
      fr: "https://drive.google.com/uc?export=download&id=1goCylGDS95EIvlcBi1Xh9NwlT1llXcgV",
      es: "https://drive.google.com/uc?export=download&id=1KkHbqRjZV16lueYyOnVEFataBdtp52Zu",
      en:"https://drive.google.com/uc?export=download&id=1OIc8cV3zLFKC9gyhcbw8AtIL4jqCd1FF",
      eu: "https://drive.google.com/uc?export=download&id=1XBew-56GZ_SL95MB0TyA-YrCbpI2lkI2"
    },
    social: [
      {id: "linkedin", url: "https://www.linkedin.com/in/ane-diaz-de-tuesta-37785252", className: "fa-linkedin"},
      {id: "twitter", url: "https://twitter.com/ane_naiz", className: "fa-twitter"},
      {id: "giphy", url: "https://giphy.com/ane_naiz", className: "giphyLogo", isSvg:true},
      {id: "github", url: "https://github.com/anediaz", className: "fa-github"},
      {id: "npm", url: "https://www.npmjs.com/~anediaz", className: "fab fa-npm"}
    ]
  },
  works:{
    developer: {
      url: "https://anediaz.github.io/araotz/#/",
      alt: "Araotz"
    },
    graphic: {
      alt: "Graphic"
    }
  },
  talks:
     [
      {
        title: "Goolge WDA 2023",
        youtubeId: "O4ch095al50",
        date: "2023/09/14"
      },
      {
        title: "BDX/IO 2022 Keynote",
        youtubeId: "ifXy9jRLWl8",
        date: "2022/07/06"
      },
      {
        title: "50inTech 2022",
        youtubeId: "VoPvX9K_Gok",
        date: "2022/07/06"
      },
      {
        title: "Sunny Tech Montpellier 2022",
        youtubeId: "l1zl0ift9q0",
        date: "2022/07/01"
      },
      {
        title: "Ladies of Code Paris 2022",
        youtubeId: "7ZLb6SQ9tTs",
        date: "2022/03/08"
      },
      {
        title: "BDX/IO 2019",
        youtubeId: "Ig5_nm5fpEY",
        date: "2019/11/15"
      },
      {
        title: "DevFest Toulouse 2018",
        youtubeId: "NInM_wndk2U",
        date: "2018/11/08"
      },
      {
        title: "Devoxx France 2018",
        youtubeId: "sxyHF5pptgY",
        date: "2018/04/19"
      },
      {
        title: "Live Scribbing",
        youtubeId: "lFkDUWcKPyU",
        date: "2018/04/20"
      },
      {
        title: "Voxxed Days Luxembourg 2018",
        youtubeId: "dYAGTPDRcD4",
        date: "2018/06/22"
      },
      {
        title: "Jug Summer Camp 2017",
        youtubeId: "EGSwp_bC3kA",
        date: "2017/09/15"
      }
    ],
  more: [{item:"languages", contentItems: ["eu", "fr", "en", "es"] }, {item: "sports", content: true}, {item: "draw", content: true}],
  webSiteInfo: {
    author: "Ane Diaz de Tuesta",
    url: "http://www.anediaz.com"
  },
}
export const IllustrationsData:Album[] = [
  { name: "Technical conferences", id: "72157713855312227" },
  { name: "Visual thinking", id: "72157711643998961" },
  { name: "Visual communication", id: "72177720301819686" },
  { name: "Basque", id: "72177720298004102" },
  { name: "Test JS Summit 2021", id: "72157718113845317" },
  { name: "Inktober 2021", id: "72157716697491682" },
  { name: "Microsoft", id: "72157719163846608" }
]
